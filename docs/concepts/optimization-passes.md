...
Groups **islands** of same-device nodes into `COMMAND_TYPE_PACK` nodes. An island
...
single fresh top-level PACK node whose `cgir::command_pack_t` sub-graph reuses the
very same node objects and their internal edges. Only the island boundary is rewired:
external edges are moved to/from the pack node (deduplicated; cross-island edges
become pack-to-pack edges), and the sub-graph's entry/exit are connected to the
island's sources/sinks.

This enables mapping to vendor-specific packing mechanisms (e.g. CUDA Graphs, HIP Graphs, Level Zero command lists).

### Sequence (sequence)

Collapses a maximal **linear chain** `u -> v -> ... -> w` of same-device
`TASK_SPAWN` PROG commands (a recorded OpenMP-task chain) into a single
`COMMAND_TYPE_PACK` node whose sub-graph is flagged `is_serial`. The runtime
replays such a pack as one "super" task (running each recorded task body serially
...
Materialization is identical to the pack pass (reuse the node objects and internal
edges in place; rewire only the boundary), and only chains with at least two
`COMMAND` members are packed.

`sequence` and `pack` both create `COMMAND_TYPE_PACK` nodes but for different
runtime mechanisms (OpenMP super-task replay vs. vendor command graphs) and are
typically enabled independently. When both run, `sequence` runs first (its packes
are then opaque boundaries to `pack`).
